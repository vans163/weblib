...
handshake(WSKey, WSExtensions, WSOptions) ->
    ...
    RespBin = weblib_http:response(101, Headers2, <<"">>),
...
%This could get expensive, need benchmark
...
decode_frame(<<>>, ZInflate) -> {incomplete, <<>>};
decode_frame(Chunk= <<Fin:1, RSV1:1, RSV2:1, RSV3:1, 
        Opcode:4, Rest/binary>>, ZInflate) ->
    ...
    case Res of
        {ok, Opcode, 1, Payload, R} ->
            PayloadDecompressed = zlib:inflate(ZInflate, <<Payload/binary,0,0,255,255>>),
            {ok, Opcode, 1, PayloadDecompressed, R};
        _ -> Res
    end.
...
encode_frame(Bin, {text_compress, ZDeflate}) -> 
    BinCompress = deflate(ZDeflate, Bin),
    encode_frame(BinCompress, 1, 1);
encode_frame(Bin, {bin_compress, ZDeflate}) -> 
    BinCompress = deflate(ZDeflate, Bin),
    encode_frame(BinCompress, 1, 2).
